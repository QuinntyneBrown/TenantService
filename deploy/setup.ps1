param (
    $resourceBaseName="tenantService$( Get-Random -Maximum 1000)",
    $location='eastus'
)
...
dotnet build ..\src\TenantService.Api\TenantService.Api.csproj

Write-Host 'Packaging API project for deployment' -ForegroundColor Green
dotnet publish ..\src\TenantService.Api\TenantService.Api.csproj --self-contained -r win-x86 -o ..\publish
